...
def call_claude(extracted_text: str, language: str = "en") -> tuple[dict, int, int]:
    ...
    try:
        ...
        logger.warning("Claude rate limit hit — retrying in 2 seconds")
        time.sleep(2)
        try:
            response = _do_call()
        ...
        logger.critical("Anthropic API key invalid or expired")
    ...
    if not raw:
        raise ValueError("EXTRACTION_FAILED|Claude returned an empty response|")

    input_tokens = response.usage.input_tokens if response.usage else 0
    output_tokens = response.usage.output_tokens if response.usage else 0

    parsed = safe_parse_claude_response(raw)
